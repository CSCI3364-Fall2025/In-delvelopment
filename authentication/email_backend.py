...
from .gmail_api import send_email_via_gmail, get_gmail_service
import logging
...
class GmailAPIEmailBackend(BaseEmailBackend):
    ...
    def __init__(self, fail_silently=False, **kwargs):
        ...
        self.fallback_backend = EmailBackend(
    ...
    def send_messages(self, email_messages):
        ...
        if not getattr(settings, 'USE_GMAIL_API', False):
            logger.info("Gmail API is disabled in settings, using fallback email backend")
        ...
        # Try each professor until one works
        for sender in professors:
            # Check if this professor has a valid Gmail token
            if get_gmail_service(sender):
                logger.info(f"Using professor {sender.email} to send emails via Gmail API")
        ...
        # If we get here, no professor had valid Gmail credentials
        logger.warning("No professors with valid Gmail credentials found, using fallback email backend")
        return self.fallback_backend.send_messages(email_messages) 